tryGet <- utils::getFromNamespace("tryGet", "rgugik")

#' taken from rgugik with modification of https -> http
...
.DEM_request = function(x) {
  ...
  base_URL = "http://mapy.geoportal.gov.pl/gprest/services/SkorowidzeFOTOMF/MapServer/1/query?"
  ...
  for (i in seq_along(x)) {
    ...
    output = rgugik:::tryGet(jsonlite::fromJSON(prepared_URL))
